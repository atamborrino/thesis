...
update event processed during the previous job. In order to persist for each job the date of the last event processed, we use a persistent storage that stores the timestamps of the last event processed of each data source (the storage system is the NoSQL database MongoDB \footfullcite{bib:mongodb}, but it can be any other simple storage system that can store an association of job names with their timestamp).
